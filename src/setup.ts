...
/**
 *
 * @param {string} applicationId - Used for identifying application.
 * @param {string} applicationSecret - Used for authenticating application.
 * @param {string} [guildId] - (Optional) pass param for making guild commands instead of global.
 * @param {[ApplicationCommand, InteractionHandler][]} commands - Commands/Interactions to setup on discord slash-command.
 * @returns {Promise<Response>} - Response from running setup on discord api.
 */
export const setup = ({
  applicationId,
  applicationSecret,
